...
namespace ConsolePollyRetry
{
    public class TwoThread : IDisposable
    {
            ...
            Func<string, string> processLine)
        {
            ...
            var reader = polly.ExecuteAsync(async () => await CreateReader(inputFilename));

            //var writer = Task.Factory.StartNew(() =>
            //    WriteOutput(outputFilename), TaskCreationOptions.LongRunning);

            var writer = polly.ExecuteAsync(async () => await CreateWriter(outputFilename));
